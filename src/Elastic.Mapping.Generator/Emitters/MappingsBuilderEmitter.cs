...
internal static class MappingsBuilderEmitter
{
	...
	private static void EmitNamespace(StringBuilder sb, TypeMappingModel model)
	{
		...
		EmitMappingsBuilderClass(sb, model);
	...
	private static void EmitMappingsBuilderClass(StringBuilder sb, TypeMappingModel model)
	{
		var indent = new string('\t', model.ContainingTypes.Length);
		var builderClassName = $"{model.TypeName}MappingsBuilder";
		var analysisPath = $"{model.TypeName}.Analysis";
		...
		sb.AppendLine($"{indent}/// <summary>Type-specific mappings builder for {model.TypeName}.</summary>");
		...
		for (var i = 0; i < props.Count; i++)
		{
			...
			// Track nested properties for additional overload generation
		...
		// Generate overloaded methods for nested properties (takes nested builder)
		...
		// Generate nested builder classes
		foreach (var (propertyName, fieldName, nestedType) in nestedBuilders)
			EmitNestedBuilderClass(sb, indent, propertyName, fieldName, nestedType);
	}

	private static void EmitAnalysisAccessor(StringBuilder sb, string indent, string analysisPath)
	{
		// Create a nested class that wraps the static Analysis class for instance access
	...
	)
	{
		...
		// Generate a method for each nested property
	...
	private static void EmitPropertyMethod(StringBuilder sb, string indent, string builderClassName, PropertyMappingModel prop)
	{
		...
		// For known field types, generate a type-constrained method
		if (factoryMethod != null)
		{
		...
		{
			// For unknown field types, generate a generic method
			sb.AppendLine($"{indent}\t/// <summary>Configure the {prop.PropertyName} field (maps to \"{prop.FieldName}\").</summary>");
	...
	private static void EmitPartialClassExtension(StringBuilder sb, TypeMappingModel model)
	{
		...
		sb.AppendLine($"{indent}{{");
		sb.AppendLine($"{indent}\t/// <summary>Creates a type-specific mappings builder for {model.TypeName}.</summary>");
		sb.AppendLine($"{indent}\tpublic static {builderClassName} MappingsBuilder() => new();");
		sb.AppendLine($"{indent}}}");
	}
}
